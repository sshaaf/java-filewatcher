...
public abstract class AbstractFileWatcher implements Runnable {
    ...
    /**
     * Set if you would like to make a recursive watcher. false by default
     * 
     * @param isRecursive
     */
     ...
     * @param fileFilter
     */
    public AbstractFileWatcher(URI filePath,
	    AbstractFileListAdapter fileListAdapter, FileFilter fileFilter) {
    ...
    /**
     * Initliazes the fileListAdapter and is invoked at the time of
     * construction.
     */
    protected void init() {
    ...
    protected void compare(File file) {
	if (file.isDirectory()) {
    ...
    /**
     * Shaaf TODO: If file is a directory need a recursive a function.
     * */
    /**
     * This method will only perform a recursive look up if the isRecursive =
     * true
     * 
	...
	for (int i = 0; i < inProcessFiles.length; i++) {
	    if (inProcessFiles[i].isDirectory() && isRecursive)
		compare(inProcessFiles[i].listFiles(getFileFilter()));
	    else {
		FileEventType fileEventType = fileListAdapter
			.compareToMasterFile(inProcessFiles[i]);
		try {
		...
		fileListAdapter.add(inProcessFiles[i],
			inProcessFiles[i].lastModified());
	    }
    ...
    /**
     * This method throws an event to all subscribers. Importantly this is not
     * asynchronous from this point.
     * 
     * @param file
     * @param fileEventType
	...
	FileEvent event = EventFactory.createEvent(getFile(), file,
		fileEventType);
	if (event != null) {
